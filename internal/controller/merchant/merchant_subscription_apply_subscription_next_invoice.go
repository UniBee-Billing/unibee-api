...
func (c *ControllerSubscription) ApplySubscriptionNextInvoice(ctx context.Context, req *subscription.ApplySubscriptionNextInvoiceReq) (res *subscription.ApplySubscriptionNextInvoiceRes, err error) {
	...
	utility.Assert(len(req.DiscountCode) > 0 || req.ApplyPromoCreditAmount > 0, "discount code or premo credit should not be null")
	if len(req.DiscountCode) > 0 || req.ApplyPromoCreditAmount > 0 {
		next.SaveSubscriptionNextInvoiceData(ctx, sub.SubscriptionId, &bean.SubscriptionNextInvoiceData{
	...
	return &subscription.ApplySubscriptionNextInvoiceRes{
